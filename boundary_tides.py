## The code in this file was borrowd from the North West Atlantic github repos. Not all of it was needed, and I've made some minor modifications 
#Original code: https://github.com/jsimkins2/nwa25


## The following functions and classes are from the 'boundary.py' file under /setup
import sys
from os import path
import warnings
...
## The following is from the /setup/tides folder. This function reads in your tpxo file and cuts out the parts along the boundary.

# From https://github.com/jsimkins2/nwa25/tree/main/setup/tides

def write_tpxo(constituents, tpxo_dir, segments, horizontal_subset,tpxo_filename):
    """
    Inputs: 
        constituents            list of int     the tidal constituents to include in model
        tpxo_dir                str             path to your tpxo tidal data
        segments                segment class   the segments to cut data out along
        horizontal_subset       dict            set of coordinates that roughly surrounds your domain
        tpxo_filename           str             the base name of your tpxo files. Varies with version number, eg. tpxo9.v1.nc

    Outputs:
        Nothing. Writes your segments to disk via a method in the segment class.
    """
    tpxo_h = (
        xarray.open_dataset(path.join(tpxo_dir, f'h_{tpxo_filename}')) ## Change to new version when we have it
    ...
    tpxo_u = (
        xarray.open_dataset(path.join(tpxo_dir, f'u_{tpxo_filename}'))
