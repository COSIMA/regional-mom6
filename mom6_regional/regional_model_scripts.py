...
def sel_hgrid_indices(field, extent):
    ...
    for i in range(field.shape[0]):
        if round(field[i].values.reshape(1)[0], 6) >= extent[0] and temp == False:
            ...
            indices.append(
                (i - 1) * 2 + 1
            )  ## This goes back a step to find the last t point that was in the bounds. Then goes forward one so that the final point to slice is on the corner
            temp = False
            break
...
def prepare_segments(
    xextent, yextent, run_year=2170, t=range(1077, 1082), base=os.getenv("PBS_JOBFS")
):
    """
    ...
    d_tracer = time_rotate(
        d_tracer.sel(time=slice(f"{run_year}-05-01", f"{run_year+1}-04-30"))
    )
    d_velocity = time_rotate(
        d_velocity.sel(time=slice(f"{run_year}-05-01", f"{run_year+1}-04-30"))
    )

    # reduce selection around target latitude
    # and remove spatial chunks (required for xesmf)
    d_tracer = d_tracer.sel(
        yt_ocean=slice(yextent[0] - 1, yextent[1] + 1),
        xt_ocean=slice(xextent[0] - 1, xextent[1] + 1),
    ).chunk({"yt_ocean": None, "xt_ocean": None})
    d_velocity = d_velocity.sel(
        yu_ocean=slice(yextent[0] - 1, yextent[1] + 1),
        xu_ocean=slice(xextent[0] - 1, xextent[1] + 1),
    ).chunk({"yu_ocean": None, "xu_ocean": None})
...
def interp_segment(segment, path, weights_exist=False, base=os.getenv("PBS_JOBFS")):
    ...
    *prepare_segments* script. The value of this variable should match
    ...
    for var in surface_vars:
        ...
        # add the y dimension
        segment_out[v] = segment_out[v].expand_dims(f"{seg_alt}_{seg}", axis=alt_axis)
        segment_out[f"dz_{v}"] = (
        ...
        encoding_dict[f"nz_{seg}_{var}"] = {"dtype": "int32"}
    ...
    with ProgressBar():
        ...
        segment_out.load().to_netcdf(
            path + f"forcing/forcing_obc_{seg}.nc",
...
def regrid_runoff(
    ocean_mask_path, hgrid_path, runoff_path, output_path, xextent, yextent
):
    """
    ...
    area["ny"] = area.ny // 2
    area["nx"] = area.nx // 2
    area = area.stack(cell=["ny", "nx"]).groupby("cell").sum().unstack("cell")
    ...
    cst_coords = (
        xr.concat((dg.y, dg.x + 360), "d").data.reshape(2, -1).T[np.flatnonzero(cst)]
    )
    ...
    dr = xr.open_dataset(runoff_path).sel(
        latitude=slice(yextent[0], yextent[1]),
        longitude=slice(
            xextent[0] + 360, xextent[1] + 360
        ),  ## need to add 360 since xextent is between -280 -> 80
    )
    res = 0.25
    ...
    runoff_coords = (
        np.c_[np.meshgrid(dr.latitude, dr.longitude, indexing="ij")].reshape(2, -1).T
    )
    # coords for cell area calculation
    corner_lat, corner_lon = np.meshgrid(
        np.deg2rad(lats), np.deg2rad(lons), indexing="ij"
    )
    Re = 6378.137e3
    runoff_areas = np.abs(
        ((corner_lon[1:, 1:] - corner_lon[:-1, :-1]) * Re**2)
        * (np.sin(corner_lat[1:, 1:]) - np.sin(corner_lat[:-1, :-1]))
    )
    ...
    runoff = xr.DataArray(
        ...
        {
            "time": dr.time,
            "latitude": dg.y.isel(longitude=0),
            "longitude": dg.x.isel(latitude=0),
        },
...
def Create_Base_Grid(
    ...
    grid_dx=1 / 50,
    grid_dy=1 / 50,
    FRE_tools_dir="/g/data/ul08/FRE_tools/bin/bin",
):
    ...
    input_args = (
        input_args + f" --xbnd {west_longitude_limit},{east_longitude_limit}"
    ...
    try:
        print(
            "MAKE HGRID",
            subprocess.run(
                [FRE_tools_dir + "/make_hgrid"] + input_args.split(" "),
